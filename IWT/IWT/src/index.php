    ...
    <?php
        ...
        if($result->num_rows>0){
            while($row=$result->fetch_assoc()){
				...
				.$row["Item_description"]."</td><td>".$row["Price"]."</td><td>".$row["Quantity"]."
				</td><td>".$row["Item_code"]."</td>";
                
                echo "<td><a href='editrecord.php? id=$row[Item_id] &name=$row[Item_name] &des=$row[Item_description]
				&price=$row[Price] &qty=$row[Quantity] &code=$row[Item_code]'>edit</a></td>";
